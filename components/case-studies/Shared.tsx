...
export const ZoomableImage = ({ src, alt, className = "", fill = true }: { src: string, alt: string, className?: string, fill?: boolean }) => {
    ...
    return (
        <>
            <div
                className={`cursor-zoom-in group relative overflow-hidden rounded-xl ${fill ? 'h-full w-full' : 'w-full'}`}
            ...
            >
                ...
                <img
                    src={src}
                    alt={alt}
                    className={`${className} ${fill ? 'h-full w-full object-cover' : 'w-full h-auto'} transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}
