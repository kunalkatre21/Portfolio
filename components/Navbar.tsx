...
export const Navbar: React.FC = () => {
  ...
  const scrollToSection = (id: string) => {
    ...
    analytics.trackNavigation(id, isMobileMenuOpen ? 'mobile-menu' : 'desktop-nav');
            ...
            {navItems.map((item) => (
              <li key={item.id}>
                <button
                  ...
                  className={`relative px-4 py-2 text-sm font-medium transition-colors duration-300 rounded-full ${activeSection === item.id
          ...
          <div className="md:hidden flex items-center pl-2">
            ...
            <button
              onClick={() => {
                ...
                analytics.trackMobileMenu('open', 'menu-button');
              }}
            ...
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex gap-4">
                ...
                <button
                  onClick={() => {
                    setIsMobileMenuOpen(false);
                    analytics.trackMobileMenu('close', 'close-button');
                  }}
                  className="p-3 rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-900 dark:text-white transition-colors"
                >
                  <X size={20} />
            ...
            <nav className="flex-1 flex flex-col justify-center gap-6 items-center">
              {navItems.map((item, i) => (
                <motion.button
                  ...
                  onClick={() => scrollToSection(item.id)}
                  className={`text-4xl font-bold transition-colors ${activeSection === item.id
